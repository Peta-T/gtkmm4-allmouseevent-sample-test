...
void AdvancedMouseTracker::setup_gesture(guint btn_id) {
    ...
    g->signal_pressed().connect(sigc::bind(sigc::mem_fun(*this, &AdvancedMouseTracker::on_mb_pressed_internal), btn_id));
    g->signal_released().connect(sigc::bind(sigc::mem_fun(*this, &AdvancedMouseTracker::on_mb_released_internal), btn_id));
...
// ========================================================================
// INTERNÍ LOGIKA (ROUTER)
// ========================================================================

void AdvancedMouseTracker::on_mb_pressed_internal(int n_press, double x, double y, guint button_id)
{
    if (n_press == 1) {
    ...
    // Router pro volání virtuálních metod
    switch (button_id) {
...
void AdvancedMouseTracker::on_mb_released_internal(int n_press, double x, double y, guint button_id)
{
    if (n_press > 1) return;

    bool was_dragging_owner = (m_active_drag_id == button_id);

    // Volání Release metod (jen pokud nebyl drag, nebo dle vaší logiky)
    // Zde voláme vždy Release pro konzistenci, nebo můžete dát podmínku !was_dragging_owner
    switch (button_id) {
    ...
    if (was_dragging_owner) {
        ...
        // Sekundární tlačítko puštěno během dragu - force end
...
void AdvancedMouseTracker::force_drag_end(std::string reason)
{
    if (m_active_drag_id != 0) {
        ...
        else if (m_active_drag_id == 2) on_MB2_drag_end(); // Note: combos might need separate end logic if desired
...
bool AdvancedMouseTracker::on_event_legacy(const Glib::RefPtr<const Gdk::Event>& event)
{
    ...
    if (gdk_event_get_event_type(c_event) == GDK_BUTTON_RELEASE)
    {
        ...
        {
            on_mb_released_internal(1, 0, 0, btn_id);
            return false;
        }
    }
    return false;
...
void AdvancedMouseTracker::on_mouse_enter_internal(double, double) {} // Placeholder
